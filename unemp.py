...
import pandas as pd
df = pd.read_csv('sc_unemployment.csv',names = ['county', 'year', 'rate'], skiprows = 1)
...
source = ColumnDataSource(data={
    'xs':list(df_final[df_final['year']==2007]['xs']),
    'ys':list(df_final[df_final['year']==2007]['ys']),
    'name':list(df_final[df_final['year']==2007]['county']),
    'rate':list(df_final[df_final['year']==2007]['rate']),
    'year':list(df_final[df_final['year']==2007]['year'])}
)
...
p = figure(
    title="South Carolina Unemployment, 2018", tools=TOOLS, plot_width=600, plot_height=500,
...
def update_plot(attr, old, new):
    ...
    yr = slider.value
    new_data = {
    'xs':list(df_final[df_final['year']==yr]['xs']),
    'ys':list(df_final[df_final['year']==yr]['ys']),
    'name':list(df_final[df_final['year']==yr]['county']),
    'rate':list(df_final[df_final['year']==yr]['rate']),
    'year':list(df_final[df_final['year']==yr]['year']),
}
